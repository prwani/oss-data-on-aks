...
locals {
  ...
  agent_pools = {
    flink = {
      name               = var.flink_node_pool_name
      vm_size            = var.flink_node_pool_vm_size
      count_of           = var.flink_node_pool_count
      mode               = "User"
      os_type            = "Linux"
      availability_zones = []
      node_labels = {
