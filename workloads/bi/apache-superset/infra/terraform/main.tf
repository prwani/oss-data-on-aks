...
locals {
  tags = merge(
    {
      blueprint   = "apache-superset-on-aks"
  ...
  agent_pools = {
    superset = {
      name               = var.superset_node_pool_name
      vm_size            = var.superset_node_pool_vm_size
      count_of           = var.superset_node_pool_count
      mode               = "User"
      os_type            = "Linux"
      availability_zones = []
      node_labels = {
      ...
      os_disk_size_gb = var.superset_node_pool_os_disk_size_gb
      upgrade_settings = {
        max_surge = "10%"
      }
    }
