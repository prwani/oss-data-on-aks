...
Use the secure portal button when you also want a private AKS control plane. It creates a VNet, private AKS API server, keyless snapshot storage, and optional Azure Bastion for private AKS API access:
...
After the secure portal deployment, use `az aks bastion` from the deployment output to open a private AKS API tunnel, then run the Kubernetes, Helm, and store-search sample steps from the docs.
