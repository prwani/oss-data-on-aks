...
Apply the Premium SSD storage class and namespace manifest before the Helm release. The storage class manifest matches the AKS built-in `managed-csi-premium` definition, so it is safe to apply on clusters that already ship that class. The namespace uses privileged Pod Security labels because the checked-in Redpanda values keep `tuning.tune_aio_events` enabled, and that tuning path creates a privileged container.
