...
locals {
  tags = merge(
    {
      blueprint   = "apache-kafka-on-aks"
      workload    = "apache-kafka"
  ...
  agent_pools = {
    kafka = {
      name               = "kafka"
      vm_size            = var.kafka_node_vm_size
      count_of           = var.kafka_node_count
      mode               = "User"
      os_type            = "Linux"
      availability_zones = []
      node_labels = {
      ...
      os_disk_size_gb = var.kafka_node_os_disk_size_gb
      upgrade_settings = {
        max_surge = "10%"
      }
    }
